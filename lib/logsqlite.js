...
class LogSqlite {  
    ...
    async find(match,channel,from,to) {
        //channel=channel||"private";
        var db = await this.connect();
        return new Promise((resolve,reject)=>{
            var filterChannel=channel=>(channel?` AND logs.channel='${channel}' `:''); 
            var filterDateFrom=from=>(from?` AND date >= '${from}' `:'');
            var filterDateTo=to=>(to?` AND date <= '${to}' `:''); 
            var fts5=query=>(query?` where logs_fts MATCH '${query}'`:'');
            var sql = `select *,date(time) as date from logs WHERE logs.logs_id IN (SELECT rowid FROM logs_fts${fts5(match)}) ${filterChannel(channel)} ${filterDateFrom(from)} ${filterDateTo(to)}`;
